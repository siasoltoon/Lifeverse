...
def setup_character(session):
    ...
    session.add_all(
        [
            Job(
                code="dev", name="Developer", min_level=1, base_salary=Decimal("1000"), work_hours=8
            ),
    ...
    session.flush()
    city = City(
        country_id=country.id, name="Test City", region="Test", population=1, cost_of_living_index=1
    )
    session.add(city)
    session.commit()
    p = PlayerService()
    a = p.register(session, "p")
    c = p.create_character(session, a.id, "Hero", country.id, city.id)
    return c, city
...
def test_economy_idempotency_and_no_negative_balance(session):
    c, _ = setup_character(session)
    e = EconomyService()
    e.credit(session, c.id, 100, "k-12345678")
    e.debit(session, c.id, 30, "k-12345679")
    e.debit(session, c.id, 30, "k-12345679")
    assert e.balance(session, c.id) == Decimal("70.00")


def test_jobs_skills_inventory_social_and_mission(session):
    c1, city = setup_character(session)
    p = PlayerService()
    a2 = p.register(session, "p2")
    c2 = p.create_character(session, a2.id, "Friend", c1.country_id, city.id)
    career = CareerService()
    job = session.query(Job).first()
    career.hire(session, c1.id, job.id)
    skill = session.query(Skill).first()
    SkillService().add_xp(session, c1.id, skill.id, 100)
    item = session.query(Item).first()
    InventoryService().add(session, c1.id, item.id, 2)
    InventoryService().remove(session, c1.id, item.id, 1)
    SocialService().relate(session, c1.id, c2.id, "friend", 20)
    mission = session.query(Mission).first()
    MissionService().start(session, c1.id, mission.id)
    MissionService().progress(session, c1.id, mission.id, 100, EconomyService())
...
def test_travel_requires_distinct_destination(session):
    ...
    other = C(
        country_id=c.country_id, name="Other", region="Test", population=1, cost_of_living_index=1
    )
    session.add(other)
    session.commit()
